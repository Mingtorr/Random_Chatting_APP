...
rendermessage=({item,index})=>{
  {
    if(index===0){ 
      ...
      if(this.state.arr[this.state.start+index-1].userkey === item.userkey)
      {
        if(this.state.userkey === item.userkey){
            ...
            <KeyboardAvoidingView style={styles.message_safe} behavior='padding' onAccessibilityAction={this.scrolltobottom} keyboardVerticalOffset={keyboardVerticalOffset}>
              <View style={styles.message_top} >
                <View style={{display:'flex',flex:0.5,flexDirection:"row",justifyContent:'center'}}>
                  <Text style={{fontFamily:"Jalnan",color:'white',fontSize:20}}>심심한사람 다모여라</Text>
                  <Text style={{fontFamily:"Jalnan",color:'white',fontSize:20}}></Text>
                </View>
              </View>
              <View style={styles.message_top2} >
                <View style={{display:'flex',flex:0.5,flexDirection:"row",justifyContent:'flex-end'}}>
                  <Text style={{fontFamily:"Jalnan",color:'black',fontSize:15,marginRight:20}}>접속 : {this.state.roomsize}명</Text>
                  <Text style={{fontFamily:"Jalnan",color:'black',fontSize:15,marginRight:20}}>참여 : 4명</Text>
                </View>
              </View>
