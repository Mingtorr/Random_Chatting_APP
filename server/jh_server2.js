...
app.post("/My_all_message_save", (req, res) => {
  ...
  const all_message_data = req.body;
    connection.query('INSERT INTO allmessage_table (allmessage_body, user_key) values (?,?)',
                    [all_message_data.my_all_message, all_message_data.user_key], function(err,rows,field){
        if(err){
            console.log(err);
        }else{
...
app.post("/Infinite_scroll", (req, res) => {
  ...
  connection.query(
      ...
      [scroll_number], function (err, rows, field) {
      if (err) {
          console.log(err);
...
io.on("connection", function(socket){
  ...
  socket.on('send_allchatroom', (data)=>{
    connection.query("SELECT allmessage_key, allmessage_time FROM allmessage_table WHERE user_key = (?) and allmessage_body = (?) ORDER BY allmessage_time DESC", 
    [data.user_key, data.message_body], function (err, rows, field){
        if (err) {
          console.log(err);
          ...
          console.log(index);
          const all_send_message = {key: index.allmessage_key, message_body: data.message_body, user_nickname: data.user_nickname, user_key: data.user_key, allmessage_time: index.allmessage_time};
