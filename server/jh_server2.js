...
io.on("connection", function(socket){
  ...
  socket.on('send_allchatroom', (data)=>{
    console.log("내가 전체 메세지 보냄");
      const index = 1
      const all_send_message = {key: index, message_body: data.message_body, user_nickname: data.user_nickname, user_key: data.user_key, allmessage_time: data.allmessage_time};
